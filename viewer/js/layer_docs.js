...
window.LAYER_DOCS = {
  ...
  "trafikverket-road-layers": { title: "Roads &amp; Highways (Trafikverket)", desc: "Live road-network layers from the Swedish Transport Administration covering the greater Gothenburg area: roadside cameras, measured traffic flow, road-surface conditions and rest stops. Served through the backend, which queries Trafikverket directly and caches for 60 s. Requires the backend server (uvicorn) and a TRAFIKVERKET_API_KEY in .env.", source: "Trafikverket Open API (datex II / trafikinfo v2)" },
  "traffic-cameras": { title: "Traffic Cameras", desc: "Roadside camera locations with their latest photo. Click a pin to open the image, which then refreshes every 30 seconds while the popup stays open.", source: "Trafikverket · Camera objecttype" },
  "traffic-flow": { title: "Traffic Flow", desc: "Measured vehicle flow rate and average speed at fixed detector sites. Circle size scales with flow; colour shows congestion — green under 200 veh/h, amber 200–600, red above 600. Refreshes every 5 minutes while active.", source: "Trafikverket · TrafficFlow objecttype" },
  "road-conditions": { title: "Road Conditions", desc: "Reported road-surface state (normal, slippery, ice, snow) per road section, with the affected road number and location. Green = normal, amber = caution, red = hazardous.", source: "Trafikverket · RoadCondition objecttype" },
  "road-parking": { title: "Rest Stops &amp; Parking", desc: "Trunk-road rest areas and parking facilities with opening status, operational status and capacity. Distinct from the Västtrafik commuter parking layer, which covers public-transport park-and-ride sites.", source: "Trafikverket · Parking objecttype" },
